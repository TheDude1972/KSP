...
{
    ///
    /// <summary>DudeLoader - Stock config changer</summary>
    /// 
    ...
    public class ConfigManager : MonoBehaviour
    {
        ...
        public static ConfigNode FindConfigNodeIn(ConfigNode src, string nodeType, string nodeName = null, string nodeTag = null)
        {
            #if debug
			if (nodeTag == null)
        ...
        public void OnGUI()
        {
            ...
            print("DudeTecCore loading part patches...");
            foreach (UrlDir.UrlConfig url in GameDatabase.Instance.root.AllConfigs)
            {
                if (url.type[0] != '@')
                {
    ...
    ///
    /// <summary>DudeTec - Part Module</summary>
    ///
    public class DudeCore : PartModule
    {
        ...
        public override void OnStart(PartModule.StartState state)
        {
            ...
            {
                Events["ExplodeAllEvent"].guiName = "Btn01";
            }
        }
        
        [KSPEvent(guiActive = true, guiName = "Btn01", active = true, guiActiveUnfocused = true, unfocusedRange = 8.0f)]
        public void ExplodeAllEvent()
        {
            ScreenMessages.PostScreenMessage("Btn01", 2, ScreenMessageStyle.UPPER_CENTER);
        }

        [KSPEvent(guiActive = true, guiName = "Btn02", active = true, guiActiveUnfocused = true, unfocusedRange = 8.0f)]
        public void ExplodeEvent()
        {
            ScreenMessages.PostScreenMessage("Btn02", 2, ScreenMessageStyle.UPPER_CENTER);
        }

        [KSPAction("Action01")]
        public void ExplodeAllAction(KSPActionParam param)
        {
            ScreenMessages.PostScreenMessage("Action01", 2, ScreenMessageStyle.UPPER_CENTER);
        }

        [KSPAction("Action02")]
        public void ExplodeAction(KSPActionParam param)
        {
            ScreenMessages.PostScreenMessage("Action02", 2, ScreenMessageStyle.UPPER_CENTER);
        }
    }
}
